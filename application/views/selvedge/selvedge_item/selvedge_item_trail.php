     ...
     <script type="text/javascript">
		...
		function addToCart(){
			$('#buy-button').click(function(){
				...
				/*细节选择*/
				var trouserstype = $('input[name="trouserstype"]:checked').prev().prev().val();
				...
				var rivet = $('input[name="rivet"]:checked').prev().prev().val();
			...
			$.ajax({
				...
				success : function(data){
					...
					if(data.code == 0){
						...
						//thickness
						var s_detail = get_by_detail_type(8, details);
						var html = '<input type="hidden" name="trouserstype-attach-price" value="0"/>';
						for(var i = 0; i < s_detail.length; ++i){
							html += 
								'<li class="option">' + 
		                            ...
		                            '<input type="radio"  name="trouserstype" value="' + s_detail[i].detail_name + '">' + 
						...
						$('#trouserstype-select .option-list').append(html);
		...
		function changePrice(obj){
			...
			attach_price += parseInt($('input[name="trouserstype-attach-price"]').val());	
